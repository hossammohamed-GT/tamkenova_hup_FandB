...
export class AdminPartnersComponent implements OnInit {
  ...
  save() { if (this.form.invalid) { this.form.markAllAsTouched(); return; } this.saving.set(true); const call = this.editing() ? this.service.update(this.editing()!.id, this.form.getRawValue()) : this.service.create(this.form.getRawValue()); call.subscribe({ next: () => { this.saving.set(false); this.editing.set(null); this.showForm.set(false); this.load(); }, error: () => this.saving.set(false) }); }
